 ...
 * @todo Double-check if it makes sense to implement CanOverwriteFiles
 * @todo Test if / how a subdirectory of a SharedDrive can be used as root
 * @todo Implement getVisibilty and setVisibility
 */
class GoogleDriveAdapter extends AbstractAdapter implements CanOverwriteFiles
{
    ...
    /**
     * The default parameters for the different endpoints
     *
     * @var array
     */
    protected $defaultParams = [
        'files' => [
            // Yes, use all drives (automatically assumed by Google after June 1st 2020)
    ...
    public function writeStream($path, $resource, Config $config)
    {
        ...
        // does not exist yet. So here a new one has to be created.
        ...
        // Create the meta-data for the upload
        $meta = [
            'enforceSingleParent' => true,
            'supportsAllDrives' => true
        ];

        // The client shall defer when the sFile is created.
        $this->getService()
            ->getClient()
            ->setDefer(true);
    ...
    public function updateStream($path, $resource, Config $config)
    {
        ...
        // Make the client not execute just yet
        $this->getService()
            ->getClient()
            ->setDefer(true);

        // Even it is an update, it wants a new fileObj
        $newFileObj = new \Google_Service_Drive_DriveFile();
        $newFileObj->setName($fileObj->getName());

        // Get an update file request object (defer is set to true above).
        $request = $this->gdUpdateFile($fileObj->getId(), $newFileObj);
    ...
    protected function gdListFiles(array $query = [])
    {
        ...
        $query = array_merge(
            [
                ...
                // Only needed until June 1st 2020
                'includeItemsFromAllDrives' => true,
                'supportsAllDrives' => true,
                'fields' => '*'
            ], $query);
     ...
     */
    protected function gdCreateFile(\Google_Service_Drive_DriveFile $fileObj)
    {
        ...
        $result = $this->getService()->files->create($fileObj,
            [
                'enforceSingleParent' => true,
                'supportsAllDrives' => true
            ]);
    ...
    protected function gdGetStream(\Google_Service_Drive_DriveFile $fileObj)
    {
        ...
        $response = $this->getService()->files->get($fileObj->id,
            [
                // To allow abuse we have to be the owner.
                'acknowledgeAbuse' => false,
                'supportsAllDrives' => true,
                // Needed to get the file as download / content
                'alt' => 'media',
     ...
     * file object.
     * Set the setDefer(true) option on the Google Client object to true if you
     * only want the object returned but the request not executed.
     *
     * @param string $id
    ...
    protected function gdUpdateFile(string $id, \Google_Service_Drive_DriveFile $new): \Google_Service_Drive_DriveFile
    {
        ...
        // Get a RequestInterface instance for the fileObj
        return $this->getService()->files->update($id, $new,
            [
                'enforceSingleParent' => true,
                'supportsAllDrives' => true,
                'fields' => '*'
    ...
    protected function gdDelete(string $id)
    {
        ...
        return $this->getService()->files->delete($id, [
            'supportsAllDrives' => true
        ]);
    }
